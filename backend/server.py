...
def login():
    ...
    # Simple sessionless demo: return user object (no JWT for brevity)
...
def create_artist():
    ...
    res = db.artists.insert_one({"name": name, "bio": bio})
...
def list_albums():
    ...
    if artist_id:
        try:
            ...
            return json_response({"error": "invalid artist_id"}, 400)
    ...
    items = list(cursor)
    total = db.albums.count_documents(filt)
...
def get_user_playlists(uid):
    ...
    # Cast to ObjectId and convert cursor to a list; sort newest first
...
@app.get("/api/songs_playlists")
def get_songs_playlist():
    pl = db.playlists.find_one({"name": "songs"})
    if not pl:
        abort(404)
